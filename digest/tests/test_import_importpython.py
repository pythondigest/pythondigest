...
class ImportPythonWeeklyTest(TestCase):
    def setUp(self):
        ...
        self.patcher = patch("digest.management.commands.import_importpython.urlopen")
        self.urlopen_mock = self.patcher.start()
        self.urlopen_mock.return_value = MockResponse(read_fixture(test_fixture))
    ...
    def test_correctly_creates_issue_urls(self):
        self.assertEqual(
            ImportPythonParser.get_issue_url(2),
            "http://importpython.com/static/files/issue2.html",
        )
        self.assertEqual(
            ImportPythonParser.get_issue_url(12),
            "http://importpython.com/newsletter/draft/12",
        )
        self.assertEqual(
            ImportPythonParser.get_issue_url(56),
    ...
    def test_correctly_parses_block(self):
        ...
        self.assertEqual(
            block["title"], "Project Jupyter and IPython Podcast Interview"
        )
        self.assertEqual(
            block["content"],
