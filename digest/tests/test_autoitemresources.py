...
class AutoImportResourceTest(TestCase):
    def test_filter(self):
        ...
        create_resource(
            title="test4", type_res="rss", in_edit=True, link="https://python4.ru"
        )
        ...
        self.assertEqual(
            AutoImportResource.objects.filter(
                type_res="twitter", in_edit=False
            ).count(),
            1,
        )
        self.assertEqual(AutoImportResource.objects.filter(type_res="rss").count(), 2)
        self.assertEqual(
            AutoImportResource.objects.filter(type_res="rss", in_edit=False).count(), 1
        )
    ...
    def test_resource_type_res(self):
        ...
        resource = create_resource(
            title="Test2", type_res="rss", link="https://python.ru2"
        )
        self.assertEqual(resource.type_res, "rss")
