...
def get_feed_items(count=10):
    return (
        Item.objects.filter(status="active", activated_at__lte=datetime.datetime.now())
...
class FavoriteItemsMixin(ContextMixin):
    def get_context_data(self, **kwargs):
        ...
        if likes_enable():
            ...
            items_score = [
                (item, item.vote_total) for item in items if item.vote_total > 0
            ]
