...
def _generate_release_item(
    ...
    name = "{} - {}".format(package_data.get("name"), package_version)
    description = (
        "{0}."
        ' Изменения описаны по ссылке <a href="{1}">{1}</a>. '
        'Скачать можно по ссылке: <a href="{2}">{2}</a>'.format(
            package_data.get("description"), link, package_data.get("link")
        )
    )
    return {
        "title": name,
...
def off_other_release_news(news, package_data):
    news.filter(
        title__startswith=package_data.get("name"),
...
def parse_rss():
    ...
    try:
        packages = {
            x.get("name").strip(): x
            for x in list(Package.objects.all().values("name", "description", "link"))
        }
